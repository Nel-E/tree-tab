	...
	fr.onload = function()
	{
		...
		LoadedWindows.forEach(function(LWin)
		{
			...
			chrome.windows.create({url: urls}, function(new_window)
			{
				...
				{
					chrome.runtime.sendMessage({command: "remotely_update_tabs", groups: LWin.groups, folders: LWin.folders, tabs: NewTabs, windowId: new_window.id}, function(response)
					{
...
function ImportMergeTabs()
{
	...
	fr.onload = function()
	{
		...
		let RefsWins = {};
		let RefsTabs = {};
		for (let LWI = 0; LWI < lw.length; LWI++)
		{ // clear previous window ids
			lw[LWI].id = "";
		}
		log(lw);
		chrome.windows.getAll({windowTypes: ['normal'], populate: true}, function(cw)
		{
			for (let CWI = 0; CWI < cw.length; CWI++)
			{ // loop Windows
				for (let LWI = 0; LWI < lw.length; LWI++)
				{ // loop Loaded Windows
					let tabsMatch = 0;
					for (let CTI = 0; CTI < cw[CWI].tabs.length; CTI++)
					{ // loop Tabs of each Current Window
						for (let LTI = 0; LTI < lw[LWI].tabs.length; LTI++)
						{ // loop Tabs of each Loaded Window
			...
			lw.forEach(function(w)
			{
				...
				{ // missing window, lets make one
					...
					chrome.windows.create({url: urls}, function(new_window)
					{
						...
						var Append = setInterval(function()
						{
							chrome.runtime.sendMessage({command: "remotely_update_tabs", groups: w.groups, folders: w.folders, tabs: NewTabs, windowId: new_window.id}, function(response)
							{
								HaveResponse = response;
							});
							if (HaveResponse || GiveUp > 900)
				...
				{ // window exists, lets add missing tabs
					...
					setTimeout(function()
					{
						...
						else
						{
							...
							var Append = setInterval(function()
							{
								chrome.runtime.sendMessage({command: "remotely_update_tabs", groups: w.groups, folders: w.folders, tabs: NewTabs, windowId: w.id}, function(response)
								{
									HaveResponse = response;
								});
								if (HaveResponse || GiveUp > 900)
