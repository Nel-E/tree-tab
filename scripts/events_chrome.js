	...
	if (message.windowId == CurrentWindowId) {
		switch(message.command) {
			case "tab_created":
				...
				if (bg.opt.append_orphan_tab == "as_child" && message.tab.openerTabId == undefined && $(".active:visible")[0]) {
				...
				// orphan case
					...
					if (bg.opt.append_orphan_tab == "bottom" || bg.opt.append_orphan_tab == "as_child") {
			...
			break;
			case "tab_attached":
// log("drop to "+bg.dt.DropToTabId);
// log("drop after "+bg.dt.DropAfterTabId);
// log("drop before "+bg.dt.DropBeforeTabId);
				AppendTab({ tab: message.tab, InsertBeforeId: bg.dt.DropBeforeTabId, InsertAfterId: bg.dt.DropAfterTabId, ParentId: bg.tabs[message.tabId].p, Append: true});
				// if ($("#"+message.tabId).parent().children().eq(bg.dt.DropToIndex)) {
					// if (bg.dt.DropAfterTabId) {
